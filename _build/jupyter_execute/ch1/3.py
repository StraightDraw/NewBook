...
Open the CMD.exe Prompt from Conda Navigator, and use the **cd** command (change directory) to get to the "newbook" folder on your local machine. For me, it looked like this:
...
````{margin}
```{warning}
My Norton firewall prevented jupyter-book from compiling properly, prevented my first push, and prevented the ghp-import script from completing. I had to click on the pop-up warnings each time and create an exception. Depending on your settings, you firewall may object more or less during the process.
```
````
...
* -p is a push
* -f makes the push forceful

The script does not work for me without all three options included. If it doesn't bonk, you're website is live.

````{warning}
All new websites take a few minutes (or hours) to propogate. Soon, you will be able to reference your URL without case sensivity, and various shortcuts will work like leaving off the .html on web references. For now, only the perfect URL will link you up, and GitHub's default point to the "index.html" probably will throw a 404 error code at you.
````
